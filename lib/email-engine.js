...
const CLEAN_DROP_HEADERS = new Set(['delivered-to','x-received','return-path','received-spf','authentication-results','dkim-signature']);
const HEADERS_ONLY_DROP_HEADERS = new Set(['x-google-dkim-signature','x-gm-message-state','x-google-smtp-source']);
function isNoisyCleanHeader(name) { return CLEAN_DROP_HEADERS.has(name) || name.startsWith('arc-'); }
function isNoisyHeadersOnlyHeader(name) { return isNoisyCleanHeader(name) || HEADERS_ONLY_DROP_HEADERS.has(name); }
...
function transformHeaders(rawHeaders, options = {}) {
  ...
  for (const header of headers) {
    ...
    if (cfg.cleanTransportHeaders && isNoisyCleanHeader(lower)) continue;
