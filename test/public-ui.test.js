...
test('browser assets use the current cache version',()=>{assert.match(html,/styles\.css\?v=6\.0/);assert.match(html,/app\.js\?v=6\.0/);assert.match(html,/v6\.0/)});
...
test('white theme is default and four screenshot-inspired themes persist',()=>{assert.match(html,/<html lang="en" data-theme="warm">/);for(const theme of ['warm','forest','ocean','midnight'])assert.match(html,new RegExp(`data-theme="${theme}"`));assert.equal((html.match(/class="theme-choice/g)||[]).length,4);assert.match(app,/localStorage\.setItem\('cmh9_theme_v6'/);assert.match(app,/localStorage\.getItem\('cmh9_theme_v6'/);assert.match(css,/--panel-gradient:/);assert.match(css,/--accent-gradient:/)});
