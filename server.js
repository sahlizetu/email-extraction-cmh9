...
const PREVIEW_BYTES = Number(process.env.RESULT_PREVIEW_BYTES || 300000);
...
app.post('/api/extract', async (req,res) => {
  ...
  try {
    ...
    const previewBuffers = []; let previewUsed = 0; let truncated = false;
    for (let index = 0; index < items.length; index++) {
      if (index > 0) {
        const separatorTake = Math.min(separatorBuffer.length, Math.max(0, PREVIEW_BYTES - previewUsed));
        if (separatorTake) { previewBuffers.push(separatorBuffer.subarray(0, separatorTake)); previewUsed += separatorTake; }
        if (separatorTake < separatorBuffer.length) { truncated = true; break; }
      }
      const item = items[index];
      const take = Math.min(item.content.length, Math.max(0, PREVIEW_BYTES - previewUsed));
      if (take) { previewBuffers.push(item.content.subarray(0, take)); previewUsed += take; }
      if (take < item.content.length || (previewUsed >= PREVIEW_BYTES && index < items.length - 1)) { truncated = true; break; }
    }
    let display = Buffer.concat(previewBuffers).toString('utf8');
    if (truncated) display += '\r\n\r\n[Preview limited to keep the browser responsive. Download contains all extracted emails.]';
    session.result = { mode, items, bytes, createdAt:Date.now() };
    res.json({ count:items.length, bytes, display, truncated, downloadUrl:'/api/download' });
